    ...
    m_ui(new Ui::MainWindow)
{
    ...
    QCoreApplication::setApplicationName("Pixymon - By uRobotics");//PIXYMON_TITLE);
    ...
    m_ui->setupUi(this);
    setWindowTitle("Pixymon - By uRobotics");//PIXYMON_TITLE);
    ...
    m_ui->actionURobot->setIcon(QIcon(":/icons/icons/urobot.png"));
    m_ui->toolBar->addAction(m_ui->actionURobot);

    updateButtons();

    m_parameters.add("Pixy start command", PT_STRING, "runprog 8",
...
void MainWindow::updateButtons()
{
    ...
    if (m_pixyDFUConnected && m_pixyConnected) // we're in programming mode
    {
        ...
        m_ui->actionURobot->setEnabled(false);
    ...
    else if (runstate==2) // we're in forced runstate
    {
        ...
        m_ui->actionURobot->setEnabled(true);
        //setEnabledActions(false);
    }
    ...
    if (state) // connect
    {
        try
        {
            ...
            else if (m_interpreter==NULL)
            {
                ...
                clearActions();
                /// \note init interpreter -> renderer -> videowidget.
...
void MainWindow::on_actionURobot_triggered()
{
    if (m_interpreter)
        m_interpreter->execute("runprog 8");
    //todo, start egen algoritme og kode.
}
